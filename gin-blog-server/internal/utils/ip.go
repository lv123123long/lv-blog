...
import (
	...
	"github.com/lionsoul2014/ip2region/binding/golang/xdb" //ip数据库查询工具
	"xojoc.pw/useragent"                                   // 用户代理解析工具
)

var IP = new(ipUtil) // 类型为 ipUtil指针
...
func (*ipUtil) GetIpAddress(c *gin.Context) (ipAddress string) {
	// 尝试从请求头中获取 X-Real-IP 字段，该字段通常由反向代理设置，表示客户端的真实 IP 地址。
	...
	if ipAddress == "" || len(ipAddress) == 0 || strings.EqualFold("unknow", ipAddress) {
	...
	// 如果获取到的 IP 地址是本地回环地址，则尝试获取外部 IP 地址。
	...
	// 如果 IP 地址长度超过 15 个字符，则截取第一个逗号之前的部分。
